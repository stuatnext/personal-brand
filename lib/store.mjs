...
// ENGINE_DATA_DIR redirects all reads/writes (used by the self-test so it
// never touches real data in data/).
export const DATA = process.env.ENGINE_DATA_DIR
  ? path.resolve(process.env.ENGINE_DATA_DIR)
  : path.join(ROOT, 'data');
...
export const COLLECTIONS = [
  ...
  'calendar', 'reviews', 'channels', 'leads', 'strategy', 'audit',
];
...
const ID_PREFIX = {
  ...
  offers: 'off', lanes: 'lane', brands: 'brand', channels: 'ch', leads: 'lead', audit: 'aud',
};
...
export function state() {
  ...
  for (const c of COLLECTIONS) {
    if (c === 'audit') continue;
    if (c === 'settings') out[c] = read('settings');
    else if (c === 'strategy') out[c] = read('strategy').values || null; // document, not a list
    else out[c] = items(c);
  }
  return out;
}
