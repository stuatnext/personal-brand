...
import { relationshipStrength } from './scoring.mjs';
...
export function todayBriefing({ limit = 7 } = {}) {
  ...
  // 8. Strong unprocessed insights.
  for (const i of insights.filter((i) => i.status === 'captured' && (i.commercialRelevance || 0) >= 4)) {
    push({
      kind: 'develop-insight', priority: 50,
      title: `Develop the insight: "${i.title}"`,
      why: `Commercial relevance ${i.commercialRelevance}/5, untouched since capture. Insights are the raw material of the whole engine.`,
  ...
  let stopDoing = null;
  if (sorted.length && content.length >= 5 && sorted[0][1] / content.length > 0.5) {
    stopDoing = {
