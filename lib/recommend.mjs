...
// Default to three actions: this is an editorial thinking tool, not a task
// manager. Callers can still ask for more (?limit= on /api/today).
export function todayBriefing({ limit = 3 } = {}) {
  const actions = [];
  ...
  for (const l of leads.slice(0, 6)) {
    ...
    push({
      ...
      expectedValue: l.pillar === 'prediction-markets' ? 'Category relationship or NEXTPredict-adjacent intel'
        : l.pillar === 'igaming' ? 'A NEXT.io-relevant industry relationship (media, sponsorship, event)'
        : 'A Strait Up Growth prospect conversation',
      confidence: 'medium',
  ...
  if (supportingShare > 0.5) {
    stopDoing = {
      ...
      why: `${Math.round(supportingShare * 100)}% of the last 30 days' published pieces sit entirely outside the core pillar lanes (Strait Up Growth / Singapore & SEA, prediction markets, iGaming & sports betting). Context lanes build colour, not authority; the pillars are starving.`,
    };
