...
// nextpredict-engine voice bible (data/voice/stuart-voice.md) and, as of
// 2026-07-15, Stuart's own LLM voice pack
// (data/voice/llm-voice-pack-2026-07-15/), kept in lockstep by hand with
// nextpredict-engine's lint-drafts.mjs OFF_VOICE list.
...
export const OFF_VOICE = [
  ...
  // added from the voice pack 2026-07-15 "Phrases to avoid"
  'inflection point',
  'this highlights',
  'broader trend',
  'the bit that stands out',
  'the angle here',
  'pick your brain',
...
export const SOFT_AVOID = ['signal', 'landscape', 'ecosystem', 'leverage'];
...
export function lint(text, { brand = 'stuart', kind = 'post' } = {}) {
  ...
  // "For me, ..." as a sentence opener is on the voice pack's avoid list;
  // mid-sentence uses ("works for me") are fine.
  if (/(^|[.!?]\s+)for me\b/im.test(text)) {
    warnings.push({ rule: 'soft-avoid', detail: '"For me, ..." as an opener is on the avoid list (voice pack 2026-07-15). State the view directly.' });
  }
