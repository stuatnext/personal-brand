...
export function scoreContent(content, { lintResult } = {}) {
  ...
  c.sugRelevance = (content.lanes || []).length ? 4 : 2;
  notes.sugRelevance = (content.lanes || []).length ? `Linked to lanes: ${(content.lanes || []).join(', ')}.` : 'Not linked to an authority lane.';
...
export function authorityScore() {
  ...
  const laneCoverage = new Set(content.flatMap((x) => x.lanes || [])).size;
  add('marketRelevance', 'Market relevance', clamp(laneCoverage / 8, 0, 1),
    `Content spans ${laneCoverage} authority lanes.`, null);
